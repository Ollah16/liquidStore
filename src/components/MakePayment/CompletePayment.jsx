...
const CompletePayment = ({ isDisabled }) => {
    return (
        <div>
            <div className='flex flex-col-reverse md:flex-row gap-y-5 justify-end gap-x-5 pt-5 border-t border-gray-500/30'>
                ...
                <button disabled={isDisabled} className={`${isDisabled ? 'cursor-not-allowed text-black/60 bg-gray-100 hover:bg-gray-200' : 'cursor-pointer text-white bg-theme hover:bg-[#004A36]'} gap-x-3 flex py-3 px-5 items-center justify-center md:justify-normal font-semibold  transition-colors duration-200 ease-in-out`}><span>Continue</span><ChevronRightIcon className='size-5' /></button>
            </div>
        </div>
    )
}
